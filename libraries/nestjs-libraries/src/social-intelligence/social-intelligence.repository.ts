...
export class SocialIntelligenceRepository {
  constructor(private readonly prisma: PrismaRepository<'$queryRawUnsafe'>) {}

  async dashboard(organizationId: string) {
    const [brands, targets, audits, ideas, plans] = await Promise.all([
      this.prisma.model.$queryRawUnsafe<any[]>('SELECT * FROM brand_profiles WHERE organization_id = $1 ORDER BY created_at DESC', organizationId),
      this.prisma.model.$queryRawUnsafe<any[]>('SELECT * FROM social_targets WHERE organization_id = $1 ORDER BY created_at DESC', organizationId),
      this.prisma.model.$queryRawUnsafe<any[]>('SELECT * FROM audit_runs WHERE organization_id = $1 ORDER BY created_at DESC LIMIT 20', organizationId),
      this.prisma.model.$queryRawUnsafe<any[]>('SELECT * FROM idea_bank WHERE organization_id = $1 ORDER BY created_at DESC LIMIT 100', organizationId),
      this.prisma.model.$queryRawUnsafe<any[]>('SELECT * FROM content_plans WHERE organization_id = $1 ORDER BY created_at DESC LIMIT 20', organizationId),
    ]);
